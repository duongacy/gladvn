...
- **Exception for Icons (DX over Purity)**: To optimize Developer Experience and avoid forcing consumers to remember `data-slot="icon"`, querying standard icons via `[&_svg]` or `[&>svg]` is **PERMITTED** to set default sizes inside icon-hosting components (like Button, Badge, Separator). However, you **MUST** reduce the CSS specificity to 0 using the `:where()` pseudo-class so consumers can safely override the size with utility classes. Example: `[:where(&_svg)]:size-4` (or `[:where(&>svg)]:size-4`). Do NOT use `[&_svg:not([class*='size-'])]` anymore as it is too restrictive.
...
- **Disabled State Interaction Lock**: Any interactive component in a disabled state MUST visually fade out (e.g., `opacity-50`) and physically lock pointer events (`pointer-events-none`). This is especially critical for elements that aren't native `<button>` tags (like `<a>` or generic divs acting as buttons) where the browser doesn't automatically block clicks. Use either Tailwind's built-in `disabled:` modifier or data attributes (`data-disabled:pointer-events-none data-disabled:opacity-50`).
