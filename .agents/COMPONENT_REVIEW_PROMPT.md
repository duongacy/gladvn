...
- [ ] **Zero Inline Comments (Không comment giải thích trong code):** Tuyệt đối KHÔNG viết các dòng comment giải thích (như `// 1. Macro mẫu mực...`, `// Mũi tên lơ lửng...`) trực tiếp vào file source code `.tsx` hoặc trong prop `code` của showcase. Code phải tự lên tiếng, giải thích chỉ được đặt ở `title` và `description` của `ExampleSection`.
