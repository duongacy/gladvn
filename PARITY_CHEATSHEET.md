...
- **Opacity**: `disabled:opacity-50` (or `has-disabled:opacity-50` on wrappers).
  - *Warning:* If a wrapper applies `opacity-50`, the child input MUST have `disabled:opacity-100` to prevent double-dimming (25% opacity).
...
This project enforces a **"Ring-on-focus-only"** UX. Invalid inputs display a red border when idle, and emit a red ring *only* when focused.

**Idle Invalid (No Focus):**
```tailwindcss
aria-invalid:border-destructive dark:aria-invalid:border-destructive/50
```
*(If the element has no native border, like a Checkbox/Switch, omit the border class or apply it appropriately).*
...
```
*(For wrappers, use `has-[...:focus-visible]:...` equivalently).*
