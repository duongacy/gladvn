import { Nn as e, cr as t, dr as n, qn as r } from "./showcase-vJrKG7HA.js";
import { t as i } from "./chevron-left-dWF9AuQX.js";
import { t as a } from "./chevron-right-B7sWI4XD.js";
var o = n(t());
function s(e) {
  return Object.prototype.toString.call(e) === `[object Object]`;
}
function c(e) {
  return s(e) || Array.isArray(e);
}
function l() {
  return !!(
    typeof window < `u` &&
    window.document &&
    window.document.createElement
  );
}
function u(e, t) {
  let n = Object.keys(e),
    r = Object.keys(t);
  return n.length !== r.length ||
    JSON.stringify(Object.keys(e.breakpoints || {})) !==
      JSON.stringify(Object.keys(t.breakpoints || {}))
    ? !1
    : n.every((n) => {
        let r = e[n],
          i = t[n];
        return typeof r == `function`
          ? `${r}` == `${i}`
          : !c(r) || !c(i)
            ? r === i
            : u(r, i);
      });
}
function d(e) {
  return e
    .concat()
    .sort((e, t) => (e.name > t.name ? 1 : -1))
    .map((e) => e.options);
}
function f(e, t) {
  if (e.length !== t.length) return !1;
  let n = d(e),
    r = d(t);
  return n.every((e, t) => {
    let n = r[t];
    return u(e, n);
  });
}
function p(e) {
  return typeof e == `number`;
}
function m(e) {
  return typeof e == `string`;
}
function h(e) {
  return typeof e == `boolean`;
}
function g(e) {
  return Object.prototype.toString.call(e) === `[object Object]`;
}
function _(e) {
  return Math.abs(e);
}
function v(e) {
  return Math.sign(e);
}
function y(e, t) {
  return _(e - t);
}
function b(e, t) {
  return e === 0 || t === 0 || _(e) <= _(t) ? 0 : _(y(_(e), _(t)) / e);
}
function x(e) {
  return Math.round(e * 100) / 100;
}
function S(e) {
  return D(e).map(Number);
}
function C(e) {
  return e[w(e)];
}
function w(e) {
  return Math.max(0, e.length - 1);
}
function T(e, t) {
  return t === w(e);
}
function E(e, t = 0) {
  return Array.from(Array(e), (e, n) => t + n);
}
function D(e) {
  return Object.keys(e);
}
function O(e, t) {
  return [e, t].reduce(
    (e, t) => (
      D(t).forEach((n) => {
        let r = e[n],
          i = t[n];
        e[n] = g(r) && g(i) ? O(r, i) : i;
      }),
      e
    ),
    {},
  );
}
function k(e, t) {
  return t.MouseEvent !== void 0 && e instanceof t.MouseEvent;
}
function A(e, t) {
  let n = { start: r, center: i, end: a };
  function r() {
    return 0;
  }
  function i(e) {
    return a(e) / 2;
  }
  function a(e) {
    return t - e;
  }
  function o(r, i) {
    return m(e) ? n[e](r) : e(t, r, i);
  }
  return { measure: o };
}
function j() {
  let e = [];
  function t(t, n, i, a = { passive: !0 }) {
    let o;
    if (`addEventListener` in t)
      (t.addEventListener(n, i, a), (o = () => t.removeEventListener(n, i, a)));
    else {
      let e = t;
      (e.addListener(i), (o = () => e.removeListener(i)));
    }
    return (e.push(o), r);
  }
  function n() {
    e = e.filter((e) => e());
  }
  let r = { add: t, clear: n };
  return r;
}
function M(e, t, n, r) {
  let i = j(),
    a = 1e3 / 60,
    o = null,
    s = 0,
    c = 0;
  function l() {
    i.add(e, `visibilitychange`, () => {
      e.hidden && m();
    });
  }
  function u() {
    (p(), i.clear());
  }
  function d(e) {
    if (!c) return;
    o || ((o = e), n(), n());
    let i = e - o;
    for (o = e, s += i; s >= a;) (n(), (s -= a));
    (r(s / a), (c &&= t.requestAnimationFrame(d)));
  }
  function f() {
    c ||= t.requestAnimationFrame(d);
  }
  function p() {
    (t.cancelAnimationFrame(c), (o = null), (s = 0), (c = 0));
  }
  function m() {
    ((o = null), (s = 0));
  }
  return { init: l, destroy: u, start: f, stop: p, update: n, render: r };
}
function ee(e, t) {
  let n = t === `rtl`,
    r = e === `y`,
    i = r ? `y` : `x`,
    a = r ? `x` : `y`,
    o = !r && n ? -1 : 1,
    s = u(),
    c = d();
  function l(e) {
    let { height: t, width: n } = e;
    return r ? t : n;
  }
  function u() {
    return r ? `top` : n ? `right` : `left`;
  }
  function d() {
    return r ? `bottom` : n ? `left` : `right`;
  }
  function f(e) {
    return e * o;
  }
  return {
    scroll: i,
    cross: a,
    startEdge: s,
    endEdge: c,
    measureSize: l,
    direction: f,
  };
}
function N(e = 0, t = 0) {
  let n = _(e - t);
  function r(t) {
    return t < e;
  }
  function i(e) {
    return e > t;
  }
  function a(e) {
    return r(e) || i(e);
  }
  function o(n) {
    return a(n) ? (r(n) ? e : t) : n;
  }
  function s(e) {
    return n ? e - n * Math.ceil((e - t) / n) : e;
  }
  return {
    length: n,
    max: t,
    min: e,
    constrain: o,
    reachedAny: a,
    reachedMax: i,
    reachedMin: r,
    removeOffset: s,
  };
}
function P(e, t, n) {
  let { constrain: r } = N(0, e),
    i = e + 1,
    a = o(t);
  function o(e) {
    return n ? _((i + e) % i) : r(e);
  }
  function s() {
    return a;
  }
  function c(e) {
    return ((a = o(e)), d);
  }
  function l(e) {
    return u().set(s() + e);
  }
  function u() {
    return P(e, s(), n);
  }
  let d = { get: s, set: c, add: l, clone: u };
  return d;
}
function te(e, t, n, r, i, a, o, s, c, l, u, d, f, p, m, g, x, S, C) {
  let { cross: w, direction: T } = e,
    E = [`INPUT`, `SELECT`, `TEXTAREA`],
    D = { passive: !1 },
    O = j(),
    A = j(),
    M = N(50, 225).constrain(p.measure(20)),
    ee = { mouse: 300, touch: 400 },
    P = { mouse: 500, touch: 600 },
    te = m ? 43 : 25,
    ne = !1,
    re = 0,
    F = 0,
    I = !1,
    L = !1,
    R = !1,
    z = !1;
  function ie(e) {
    if (!C) return;
    function n(t) {
      (h(C) || C(e, t)) && le(t);
    }
    let r = t;
    O.add(r, `dragstart`, (e) => e.preventDefault(), D)
      .add(r, `touchmove`, () => void 0, D)
      .add(r, `touchend`, () => void 0)
      .add(r, `touchstart`, n)
      .add(r, `mousedown`, n)
      .add(r, `touchcancel`, V)
      .add(r, `contextmenu`, V)
      .add(r, `click`, de, !0);
  }
  function ae() {
    (O.clear(), A.clear());
  }
  function oe() {
    let e = z ? n : t;
    A.add(e, `touchmove`, ue, D)
      .add(e, `touchend`, V)
      .add(e, `mousemove`, ue, D)
      .add(e, `mouseup`, V);
  }
  function se(e) {
    let t = e.nodeName || ``;
    return E.includes(t);
  }
  function B() {
    return (m ? P : ee)[z ? `mouse` : `touch`];
  }
  function ce(e, t) {
    let n = d.add(v(e) * -1),
      r = u.byDistance(e, !m).distance;
    return m || _(e) < M
      ? r
      : x && t
        ? r * 0.5
        : u.byIndex(n.get(), 0).distance;
  }
  function le(e) {
    let t = k(e, r);
    ((z = t),
      (R = m && t && !e.buttons && ne),
      (ne = y(i.get(), o.get()) >= 2),
      !(t && e.button !== 0) &&
        (se(e.target) ||
          ((I = !0),
          a.pointerDown(e),
          l.useFriction(0).useDuration(0),
          i.set(o),
          oe(),
          (re = a.readPoint(e)),
          (F = a.readPoint(e, w)),
          f.emit(`pointerDown`))));
  }
  function ue(e) {
    if (!k(e, r) && e.touches.length >= 2) return V(e);
    let t = a.readPoint(e),
      n = a.readPoint(e, w),
      o = y(t, re),
      c = y(n, F);
    if (!L && !z && (!e.cancelable || ((L = o > c), !L))) return V(e);
    let u = a.pointerMove(e);
    (o > g && (R = !0),
      l.useFriction(0.3).useDuration(0.75),
      s.start(),
      i.add(T(u)),
      e.preventDefault());
  }
  function V(e) {
    let t = u.byDistance(0, !1).index !== d.get(),
      n = a.pointerUp(e) * B(),
      r = ce(T(n), t),
      i = b(n, r),
      o = te - 10 * i,
      s = S + i / 50;
    ((L = !1),
      (I = !1),
      A.clear(),
      l.useDuration(o).useFriction(s),
      c.distance(r, !m),
      (z = !1),
      f.emit(`pointerUp`));
  }
  function de(e) {
    R &&= (e.stopPropagation(), e.preventDefault(), !1);
  }
  function fe() {
    return I;
  }
  return { init: ie, destroy: ae, pointerDown: fe };
}
function ne(e, t) {
  let n, r;
  function i(e) {
    return e.timeStamp;
  }
  function a(n, r) {
    let i = `client${(r || e.scroll) === `x` ? `X` : `Y`}`;
    return (k(n, t) ? n : n.touches[0])[i];
  }
  function o(e) {
    return ((n = e), (r = e), a(e));
  }
  function s(e) {
    let t = a(e) - a(r),
      o = i(e) - i(n) > 170;
    return ((r = e), o && (n = e), t);
  }
  function c(e) {
    if (!n || !r) return 0;
    let t = a(r) - a(n),
      o = i(e) - i(n),
      s = i(e) - i(r) > 170,
      c = t / o;
    return o && !s && _(c) > 0.1 ? c : 0;
  }
  return { pointerDown: o, pointerMove: s, pointerUp: c, readPoint: a };
}
function re() {
  function e(e) {
    let { offsetTop: t, offsetLeft: n, offsetWidth: r, offsetHeight: i } = e;
    return {
      top: t,
      right: n + r,
      bottom: t + i,
      left: n,
      width: r,
      height: i,
    };
  }
  return { measure: e };
}
function F(e) {
  function t(t) {
    return (t / 100) * e;
  }
  return { measure: t };
}
function I(e, t, n, r, i, a, o) {
  let s = [e].concat(r),
    c,
    l,
    u = [],
    d = !1;
  function f(e) {
    return i.measureSize(o.measure(e));
  }
  function p(i) {
    if (!a) return;
    ((l = f(e)), (u = r.map(f)));
    function o(n) {
      for (let a of n) {
        if (d) return;
        let n = a.target === e,
          o = r.indexOf(a.target),
          s = n ? l : u[o];
        if (_(f(n ? e : r[o]) - s) >= 0.5) {
          (i.reInit(), t.emit(`resize`));
          break;
        }
      }
    }
    ((c = new ResizeObserver((e) => {
      (h(a) || a(i, e)) && o(e);
    })),
      n.requestAnimationFrame(() => {
        s.forEach((e) => c.observe(e));
      }));
  }
  function m() {
    ((d = !0), c && c.disconnect());
  }
  return { init: p, destroy: m };
}
function L(e, t, n, r, i, a) {
  let o = 0,
    s = 0,
    c = i,
    l = a,
    u = e.get(),
    d = 0;
  function f() {
    let t = r.get() - e.get(),
      i = !c,
      a = 0;
    return (
      i
        ? ((o = 0), n.set(r), e.set(r), (a = t))
        : (n.set(e), (o += t / c), (o *= l), (u += o), e.add(o), (a = u - d)),
      (s = v(a)),
      (d = u),
      C
    );
  }
  function p() {
    return _(r.get() - t.get()) < 0.001;
  }
  function m() {
    return c;
  }
  function h() {
    return s;
  }
  function g() {
    return o;
  }
  function y() {
    return x(i);
  }
  function b() {
    return S(a);
  }
  function x(e) {
    return ((c = e), C);
  }
  function S(e) {
    return ((l = e), C);
  }
  let C = {
    direction: h,
    duration: m,
    velocity: g,
    seek: f,
    settled: p,
    useBaseFriction: b,
    useBaseDuration: y,
    useFriction: S,
    useDuration: x,
  };
  return C;
}
function R(e, t, n, r, i) {
  let a = i.measure(10),
    o = i.measure(50),
    s = N(0.1, 0.99),
    c = !1;
  function l() {
    return !(c || !e.reachedAny(n.get()) || !e.reachedAny(t.get()));
  }
  function u(i) {
    if (!l()) return;
    let c = _(e[e.reachedMin(t.get()) ? `min` : `max`] - t.get()),
      u = n.get() - t.get(),
      d = s.constrain(c / o);
    (n.subtract(u * d),
      !i &&
        _(u) < a &&
        (n.set(e.constrain(n.get())), r.useDuration(25).useBaseFriction()));
  }
  function d(e) {
    c = !e;
  }
  return { shouldConstrain: l, constrain: u, toggleActive: d };
}
function z(e, t, n, r, i) {
  let a = N(-t + e, 0),
    o = d(),
    s = u(),
    c = f();
  function l(e, t) {
    return y(e, t) <= 1;
  }
  function u() {
    let e = o[0],
      t = C(o);
    return N(o.lastIndexOf(e), o.indexOf(t) + 1);
  }
  function d() {
    return n
      .map((e, t) => {
        let { min: r, max: i } = a,
          o = a.constrain(e),
          s = !t,
          c = T(n, t);
        return s ? i : c || l(r, o) ? r : l(i, o) ? i : o;
      })
      .map((e) => parseFloat(e.toFixed(3)));
  }
  function f() {
    if (t <= e + i) return [a.max];
    if (r === `keepSnaps`) return o;
    let { min: n, max: c } = s;
    return o.slice(n, c);
  }
  return { snapsContained: c, scrollContainLimit: s };
}
function ie(e, t, n) {
  let r = t[0];
  return { limit: N(n ? r - e : C(t), r) };
}
function ae(e, t, n, r) {
  let i = 0.1,
    { reachedMin: a, reachedMax: o } = N(t.min + i, t.max + i);
  function s(e) {
    return e === 1 ? o(n.get()) : e === -1 ? a(n.get()) : !1;
  }
  function c(t) {
    if (!s(t)) return;
    let n = t * -1 * e;
    r.forEach((e) => e.add(n));
  }
  return { loop: c };
}
function oe(e) {
  let { max: t, length: n } = e;
  function r(e) {
    let r = e - t;
    return n ? r / -n : 0;
  }
  return { get: r };
}
function se(e, t, n, r, i) {
  let { startEdge: a, endEdge: o } = e,
    { groupSlides: s } = i,
    c = d().map(t.measure),
    l = f(),
    u = p();
  function d() {
    return s(r)
      .map((e) => C(e)[o] - e[0][a])
      .map(_);
  }
  function f() {
    return r.map((e) => n[a] - e[a]).map((e) => -_(e));
  }
  function p() {
    return s(l)
      .map((e) => e[0])
      .map((e, t) => e + c[t]);
  }
  return { snaps: l, snapsAligned: u };
}
function B(e, t, n, r, i, a) {
  let { groupSlides: o } = i,
    { min: s, max: c } = r,
    l = u();
  function u() {
    let r = o(a),
      i = !e || t === `keepSnaps`;
    return n.length === 1
      ? [a]
      : i
        ? r
        : r.slice(s, c).map((e, t, n) => {
            let r = !t,
              i = T(n, t);
            return r ? E(C(n[0]) + 1) : i ? E(w(a) - C(n)[0] + 1, C(n)[0]) : e;
          });
  }
  return { slideRegistry: l };
}
function ce(e, t, n, r, i) {
  let { reachedAny: a, removeOffset: o, constrain: s } = r;
  function c(e) {
    return e.concat().sort((e, t) => _(e) - _(t))[0];
  }
  function l(n) {
    let r = e ? o(n) : s(n),
      { index: i } = t
        .map((e, t) => ({ diff: u(e - r, 0), index: t }))
        .sort((e, t) => _(e.diff) - _(t.diff))[0];
    return { index: i, distance: r };
  }
  function u(t, r) {
    let i = [t, t + n, t - n];
    if (!e) return t;
    if (!r) return c(i);
    let a = i.filter((e) => v(e) === r);
    return a.length ? c(a) : C(i) - n;
  }
  function d(e, n) {
    return { index: e, distance: u(t[e] - i.get(), n) };
  }
  function f(n, r) {
    let o = i.get() + n,
      { index: s, distance: c } = l(o),
      d = !e && a(o);
    return !r || d
      ? { index: s, distance: n }
      : { index: s, distance: n + u(t[s] - c, 0) };
  }
  return { byDistance: f, byIndex: d, shortcut: u };
}
function le(e, t, n, r, i, a, o) {
  function s(i) {
    let s = i.distance,
      c = i.index !== t.get();
    (a.add(s),
      s && (r.duration() ? e.start() : (e.update(), e.render(1), e.update())),
      c && (n.set(t.get()), t.set(i.index), o.emit(`select`)));
  }
  function c(e, t) {
    s(i.byDistance(e, t));
  }
  function l(e, n) {
    let r = t.clone().set(e);
    s(i.byIndex(r.get(), n));
  }
  return { distance: c, index: l };
}
function ue(e, t, n, r, i, a, o, s) {
  let c = { passive: !0, capture: !0 },
    l = 0;
  function u(u) {
    if (!s) return;
    function f(t) {
      if (new Date().getTime() - l > 10) return;
      (o.emit(`slideFocusStart`), (e.scrollLeft = 0));
      let a = n.findIndex((e) => e.includes(t));
      p(a) && (i.useDuration(0), r.index(a, 0), o.emit(`slideFocus`));
    }
    (a.add(document, `keydown`, d, !1),
      t.forEach((e, t) => {
        a.add(
          e,
          `focus`,
          (e) => {
            (h(s) || s(u, e)) && f(t);
          },
          c,
        );
      }));
  }
  function d(e) {
    e.code === `Tab` && (l = new Date().getTime());
  }
  return { init: u };
}
function V(e) {
  let t = e;
  function n() {
    return t;
  }
  function r(e) {
    t = o(e);
  }
  function i(e) {
    t += o(e);
  }
  function a(e) {
    t -= o(e);
  }
  function o(e) {
    return p(e) ? e : e.get();
  }
  return { get: n, set: r, add: i, subtract: a };
}
function de(e, t) {
  let n = e.scroll === `x` ? o : s,
    r = t.style,
    i = null,
    a = !1;
  function o(e) {
    return `translate3d(${e}px,0px,0px)`;
  }
  function s(e) {
    return `translate3d(0px,${e}px,0px)`;
  }
  function c(t) {
    if (a) return;
    let o = x(e.direction(t));
    o !== i && ((r.transform = n(o)), (i = o));
  }
  function l(e) {
    a = !e;
  }
  function u() {
    a ||
      ((r.transform = ``),
      t.getAttribute(`style`) || t.removeAttribute(`style`));
  }
  return { clear: u, to: c, toggleActive: l };
}
function fe(e, t, n, r, i, a, o, s, c) {
  let l = 0.5,
    u = S(i),
    d = S(i).reverse(),
    f = _().concat(v());
  function p(e, t) {
    return e.reduce((e, t) => e - i[t], t);
  }
  function m(e, t) {
    return e.reduce((e, n) => (p(e, t) > 0 ? e.concat([n]) : e), []);
  }
  function h(e) {
    return a.map((n, i) => ({ start: n - r[i] + l + e, end: n + t - l + e }));
  }
  function g(t, r, i) {
    let a = h(r);
    return t.map((t) => {
      let r = i ? 0 : -n,
        o = i ? n : 0,
        l = i ? `end` : `start`,
        u = a[t][l];
      return {
        index: t,
        loopPoint: u,
        slideLocation: V(-1),
        translate: de(e, c[t]),
        target: () => (s.get() > u ? r : o),
      };
    });
  }
  function _() {
    let e = o[0];
    return g(m(d, e), n, !1);
  }
  function v() {
    let e = t - o[0] - 1;
    return g(m(u, e), -n, !0);
  }
  function y() {
    return f.every(
      ({ index: e }) =>
        p(
          u.filter((t) => t !== e),
          t,
        ) <= 0.1,
    );
  }
  function b() {
    f.forEach((e) => {
      let { target: t, translate: n, slideLocation: r } = e,
        i = t();
      i !== r.get() && (n.to(i), r.set(i));
    });
  }
  function x() {
    f.forEach((e) => e.translate.clear());
  }
  return { canLoop: y, clear: x, loop: b, loopPoints: f };
}
function pe(e, t, n) {
  let r,
    i = !1;
  function a(a) {
    if (!n) return;
    function o(e) {
      for (let n of e)
        if (n.type === `childList`) {
          (a.reInit(), t.emit(`slidesChanged`));
          break;
        }
    }
    ((r = new MutationObserver((e) => {
      i || ((h(n) || n(a, e)) && o(e));
    })),
      r.observe(e, { childList: !0 }));
  }
  function o() {
    (r && r.disconnect(), (i = !0));
  }
  return { init: a, destroy: o };
}
function me(e, t, n, r) {
  let i = {},
    a = null,
    o = null,
    s,
    c = !1;
  function l() {
    ((s = new IntersectionObserver(
      (e) => {
        c ||
          (e.forEach((e) => {
            let n = t.indexOf(e.target);
            i[n] = e;
          }),
          (a = null),
          (o = null),
          n.emit(`slidesInView`));
      },
      { root: e.parentElement, threshold: r },
    )),
      t.forEach((e) => s.observe(e)));
  }
  function u() {
    (s && s.disconnect(), (c = !0));
  }
  function d(e) {
    return D(i).reduce((t, n) => {
      let r = parseInt(n),
        { isIntersecting: a } = i[r];
      return (((e && a) || (!e && !a)) && t.push(r), t);
    }, []);
  }
  function f(e = !0) {
    if (e && a) return a;
    if (!e && o) return o;
    let t = d(e);
    return (e && (a = t), e || (o = t), t);
  }
  return { init: l, destroy: u, get: f };
}
function he(e, t, n, r, i, a) {
  let { measureSize: o, startEdge: s, endEdge: c } = e,
    l = n[0] && i,
    u = m(),
    d = h(),
    f = n.map(o),
    p = g();
  function m() {
    if (!l) return 0;
    let e = n[0];
    return _(t[s] - e[s]);
  }
  function h() {
    if (!l) return 0;
    let e = a.getComputedStyle(C(r));
    return parseFloat(e.getPropertyValue(`margin-${c}`));
  }
  function g() {
    return n
      .map((e, t, n) => {
        let r = !t,
          i = T(n, t);
        return r ? f[t] + u : i ? f[t] + d : n[t + 1][s] - e[s];
      })
      .map(_);
  }
  return { slideSizes: f, slideSizesWithGaps: p, startGap: u, endGap: d };
}
function ge(e, t, n, r, i, a, o, s, c) {
  let { startEdge: l, endEdge: u, direction: d } = e,
    f = p(n);
  function m(e, t) {
    return S(e)
      .filter((e) => e % t === 0)
      .map((n) => e.slice(n, n + t));
  }
  function h(e) {
    return e.length
      ? S(e)
          .reduce((n, f, p) => {
            let m = C(n) || 0,
              h = m === 0,
              g = f === w(e),
              v = i[l] - a[m][l],
              y = i[l] - a[f][u],
              b = !r && h ? d(o) : 0,
              x = _(y - (!r && g ? d(s) : 0) - (v + b));
            return (p && x > t + c && n.push(f), g && n.push(e.length), n);
          }, [])
          .map((t, n, r) => {
            let i = Math.max(r[n - 1] || 0);
            return e.slice(i, t);
          })
      : [];
  }
  function g(e) {
    return f ? m(e, n) : h(e);
  }
  return { groupSlides: g };
}
function H(e, t, n, r, i, a, o) {
  let {
      align: s,
      axis: c,
      direction: l,
      startIndex: u,
      loop: d,
      duration: f,
      dragFree: p,
      dragThreshold: m,
      inViewThreshold: h,
      slidesToScroll: g,
      skipSnaps: _,
      containScroll: v,
      watchResize: y,
      watchSlides: b,
      watchDrag: x,
      watchFocus: T,
    } = a,
    E = re(),
    D = E.measure(t),
    O = n.map(E.measure),
    k = ee(c, l),
    N = k.measureSize(D),
    H = F(N),
    _e = A(s, N),
    ve = !d && !!v,
    {
      slideSizes: ye,
      slideSizesWithGaps: be,
      startGap: U,
      endGap: xe,
    } = he(k, D, O, n, d || !!v, i),
    W = ge(k, N, g, d, D, O, U, xe, 2),
    { snaps: Se, snapsAligned: G } = se(k, _e, D, O, W),
    K = -C(Se) + C(be),
    { snapsContained: Ce, scrollContainLimit: we } = z(N, K, G, v, 2),
    q = ve ? Ce : G,
    { limit: J } = ie(K, q, d),
    Y = P(w(q), u, d),
    Te = Y.clone(),
    Ee = S(n),
    De = ({
      dragHandler: e,
      scrollBody: t,
      scrollBounds: n,
      options: { loop: r },
    }) => {
      (r || n.constrain(e.pointerDown()), t.seek());
    },
    Oe = (
      {
        scrollBody: e,
        translate: t,
        location: n,
        offsetLocation: r,
        previousLocation: i,
        scrollLooper: a,
        slideLooper: o,
        dragHandler: s,
        animation: c,
        eventHandler: l,
        scrollBounds: u,
        options: { loop: d },
      },
      f,
    ) => {
      let p = e.settled(),
        m = !u.shouldConstrain(),
        h = d ? p : p && m,
        g = h && !s.pointerDown();
      g && c.stop();
      let _ = n.get() * f + i.get() * (1 - f);
      (r.set(_),
        d && (a.loop(e.direction()), o.loop()),
        t.to(r.get()),
        g && l.emit(`settle`),
        h || l.emit(`scroll`));
    },
    ke = M(
      r,
      i,
      () => De(Be),
      (e) => Oe(Be, e),
    ),
    Ae = 0.68,
    X = q[Y.get()],
    je = V(X),
    Me = V(X),
    Z = V(X),
    Q = V(X),
    $ = L(je, Z, Me, Q, f, Ae),
    Ne = ce(d, q, K, J, Q),
    Pe = le(ke, Y, Te, $, Ne, Q, o),
    Fe = oe(J),
    Ie = j(),
    Le = me(t, n, o, h),
    { slideRegistry: Re } = B(ve, v, q, we, W, Ee),
    ze = ue(e, n, Re, Pe, $, Ie, o, T),
    Be = {
      ownerDocument: r,
      ownerWindow: i,
      eventHandler: o,
      containerRect: D,
      slideRects: O,
      animation: ke,
      axis: k,
      dragHandler: te(
        k,
        e,
        r,
        i,
        Q,
        ne(k, i),
        je,
        ke,
        Pe,
        $,
        Ne,
        Y,
        o,
        H,
        p,
        m,
        _,
        Ae,
        x,
      ),
      eventStore: Ie,
      percentOfView: H,
      index: Y,
      indexPrevious: Te,
      limit: J,
      location: je,
      offsetLocation: Z,
      previousLocation: Me,
      options: a,
      resizeHandler: I(t, o, i, n, k, y, E),
      scrollBody: $,
      scrollBounds: R(J, Z, Q, $, H),
      scrollLooper: ae(K, J, Z, [je, Z, Me, Q]),
      scrollProgress: Fe,
      scrollSnapList: q.map(Fe.get),
      scrollSnaps: q,
      scrollTarget: Ne,
      scrollTo: Pe,
      slideLooper: fe(k, N, K, ye, be, Se, q, Z, n),
      slideFocus: ze,
      slidesHandler: pe(t, o, b),
      slidesInView: Le,
      slideIndexes: Ee,
      slideRegistry: Re,
      slidesToScroll: W,
      target: Q,
      translate: de(k, t),
    };
  return Be;
}
function _e() {
  let e = {},
    t;
  function n(e) {
    t = e;
  }
  function r(t) {
    return e[t] || [];
  }
  function i(e) {
    return (r(e).forEach((n) => n(t, e)), c);
  }
  function a(t, n) {
    return ((e[t] = r(t).concat([n])), c);
  }
  function o(t, n) {
    return ((e[t] = r(t).filter((e) => e !== n)), c);
  }
  function s() {
    e = {};
  }
  let c = { init: n, emit: i, off: o, on: a, clear: s };
  return c;
}
var ve = {
  align: `center`,
  axis: `x`,
  container: null,
  slides: null,
  containScroll: `trimSnaps`,
  direction: `ltr`,
  slidesToScroll: 1,
  inViewThreshold: 0,
  breakpoints: {},
  dragFree: !1,
  dragThreshold: 10,
  loop: !1,
  skipSnaps: !1,
  duration: 25,
  startIndex: 0,
  active: !0,
  watchDrag: !0,
  watchResize: !0,
  watchSlides: !0,
  watchFocus: !0,
};
function ye(e) {
  function t(e, t) {
    return O(e, t || {});
  }
  function n(n) {
    let r = n.breakpoints || {};
    return t(
      n,
      D(r)
        .filter((t) => e.matchMedia(t).matches)
        .map((e) => r[e])
        .reduce((e, n) => t(e, n), {}),
    );
  }
  function r(t) {
    return t
      .map((e) => D(e.breakpoints || {}))
      .reduce((e, t) => e.concat(t), [])
      .map(e.matchMedia);
  }
  return { mergeOptions: t, optionsAtMedia: n, optionsMediaQueries: r };
}
function be(e) {
  let t = [];
  function n(n, r) {
    return (
      (t = r.filter(({ options: t }) => e.optionsAtMedia(t).active !== !1)),
      t.forEach((t) => t.init(n, e)),
      r.reduce((e, t) => Object.assign(e, { [t.name]: t }), {})
    );
  }
  function r() {
    t = t.filter((e) => e.destroy());
  }
  return { init: n, destroy: r };
}
function U(e, t, n) {
  let r = e.ownerDocument,
    i = r.defaultView,
    a = ye(i),
    o = be(a),
    s = j(),
    c = _e(),
    { mergeOptions: l, optionsAtMedia: u, optionsMediaQueries: d } = a,
    { on: f, off: p, emit: h } = c,
    g = O,
    _ = !1,
    v,
    y = l(ve, U.globalOptions),
    b = l(y),
    x = [],
    S,
    C,
    w;
  function T() {
    let { container: t, slides: n } = b;
    C = (m(t) ? e.querySelector(t) : t) || e.children[0];
    let r = m(n) ? C.querySelectorAll(n) : n;
    w = [].slice.call(r || C.children);
  }
  function E(t) {
    let n = H(e, C, w, r, i, t, c);
    return t.loop && !n.slideLooper.canLoop()
      ? E(Object.assign({}, t, { loop: !1 }))
      : n;
  }
  function D(e, t) {
    _ ||
      ((y = l(y, e)),
      (b = u(y)),
      (x = t || x),
      T(),
      (v = E(b)),
      d([y, ...x.map(({ options: e }) => e)]).forEach((e) =>
        s.add(e, `change`, O),
      ),
      b.active &&
        (v.translate.to(v.location.get()),
        v.animation.init(),
        v.slidesInView.init(),
        v.slideFocus.init(B),
        v.eventHandler.init(B),
        v.resizeHandler.init(B),
        v.slidesHandler.init(B),
        v.options.loop && v.slideLooper.loop(),
        C.offsetParent && w.length && v.dragHandler.init(B),
        (S = o.init(B, x))));
  }
  function O(e, t) {
    let n = F();
    (k(), D(l({ startIndex: n }, e), t), c.emit(`reInit`));
  }
  function k() {
    (v.dragHandler.destroy(),
      v.eventStore.clear(),
      v.translate.clear(),
      v.slideLooper.clear(),
      v.resizeHandler.destroy(),
      v.slidesHandler.destroy(),
      v.slidesInView.destroy(),
      v.animation.destroy(),
      o.destroy(),
      s.clear());
  }
  function A() {
    _ || ((_ = !0), s.clear(), k(), c.emit(`destroy`), c.clear());
  }
  function M(e, t, n) {
    !b.active ||
      _ ||
      (v.scrollBody.useBaseFriction().useDuration(t === !0 ? 0 : b.duration),
      v.scrollTo.index(e, n || 0));
  }
  function ee(e) {
    M(v.index.add(1).get(), e, -1);
  }
  function N(e) {
    M(v.index.add(-1).get(), e, 1);
  }
  function P() {
    return v.index.add(1).get() !== F();
  }
  function te() {
    return v.index.add(-1).get() !== F();
  }
  function ne() {
    return v.scrollSnapList;
  }
  function re() {
    return v.scrollProgress.get(v.offsetLocation.get());
  }
  function F() {
    return v.index.get();
  }
  function I() {
    return v.indexPrevious.get();
  }
  function L() {
    return v.slidesInView.get();
  }
  function R() {
    return v.slidesInView.get(!1);
  }
  function z() {
    return S;
  }
  function ie() {
    return v;
  }
  function ae() {
    return e;
  }
  function oe() {
    return C;
  }
  function se() {
    return w;
  }
  let B = {
    canScrollNext: P,
    canScrollPrev: te,
    containerNode: oe,
    internalEngine: ie,
    destroy: A,
    off: p,
    on: f,
    emit: h,
    plugins: z,
    previousScrollSnap: I,
    reInit: g,
    rootNode: ae,
    scrollNext: ee,
    scrollPrev: N,
    scrollProgress: re,
    scrollSnapList: ne,
    scrollTo: M,
    selectedScrollSnap: F,
    slideNodes: se,
    slidesInView: L,
    slidesNotInView: R,
  };
  return (D(t, n), setTimeout(() => c.emit(`init`), 0), B);
}
U.globalOptions = void 0;
function xe(e = {}, t = []) {
  let n = (0, o.useRef)(e),
    r = (0, o.useRef)(t),
    [i, a] = (0, o.useState)(),
    [s, c] = (0, o.useState)(),
    d = (0, o.useCallback)(() => {
      i && i.reInit(n.current, r.current);
    }, [i]);
  return (
    (0, o.useEffect)(() => {
      u(n.current, e) || ((n.current = e), d());
    }, [e, d]),
    (0, o.useEffect)(() => {
      f(r.current, t) || ((r.current = t), d());
    }, [t, d]),
    (0, o.useEffect)(() => {
      if (l() && s) {
        U.globalOptions = xe.globalOptions;
        let e = U(s, n.current, r.current);
        return (a(e), () => e.destroy());
      } else a(void 0);
    }, [s, a]),
    [c, i]
  );
}
xe.globalOptions = void 0;
var W = e(),
  Se = o.createContext(null);
function G() {
  let e = o.useContext(Se);
  if (!e) throw Error(`useCarousel must be used within a <Carousel />`);
  return e;
}
var K = o.forwardRef(
  (
    {
      orientation: e = `horizontal`,
      opts: t,
      setApi: n,
      plugins: i,
      className: a,
      children: s,
      ...c
    },
    l,
  ) => {
    let [u, d] = xe({ ...t, axis: e === `horizontal` ? `x` : `y` }, i),
      [f, p] = o.useState(!1),
      [m, h] = o.useState(!1),
      g = o.useCallback((e) => {
        e && (p(e.canScrollPrev()), h(e.canScrollNext()));
      }, []),
      _ = o.useCallback(() => {
        d?.scrollPrev();
      }, [d]),
      v = o.useCallback(() => {
        d?.scrollNext();
      }, [d]),
      y = o.useCallback(
        (t) => {
          if (e === `horizontal`)
            switch (t.key) {
              case `ArrowLeft`:
                (t.preventDefault(), _());
                break;
              case `ArrowRight`:
                (t.preventDefault(), v());
                break;
            }
          else
            switch (t.key) {
              case `ArrowUp`:
                (t.preventDefault(), _());
                break;
              case `ArrowDown`:
                (t.preventDefault(), v());
                break;
            }
        },
        [_, v, e],
      );
    return (
      o.useEffect(() => {
        !d || !n || n(d);
      }, [d, n]),
      o.useEffect(() => {
        if (d)
          return (
            g(d),
            d.on(`reInit`, g),
            d.on(`select`, g),
            () => {
              (d?.off(`reInit`, g), d?.off(`select`, g));
            }
          );
      }, [d, g]),
      (0, W.jsx)(Se.Provider, {
        value: {
          carouselRef: u,
          api: d,
          opts: t,
          orientation: e,
          scrollPrev: _,
          scrollNext: v,
          canScrollPrev: f,
          canScrollNext: m,
        },
        children: (0, W.jsx)(`div`, {
          ref: l,
          onKeyDownCapture: y,
          className: r(`group/carousel @container/carousel relative`, a),
          role: `region`,
          "aria-roledescription": `carousel`,
          "data-slot": `carousel`,
          "data-orientation": e,
          ...c,
          children: s,
        }),
      })
    );
  },
);
K.displayName = `Carousel`;
var Ce = o.forwardRef(({ className: e, ...t }, n) => {
  let { carouselRef: i } = G();
  return (0, W.jsx)(`div`, {
    ref: o.useCallback(
      (e) => {
        (i(e), typeof n == `function` ? n(e) : n && (n.current = e));
      },
      [i, n],
    ),
    className: `overflow-hidden`,
    "data-slot": `carousel-content`,
    children: (0, W.jsx)(`div`, {
      className: r(
        `flex group-data-[orientation=vertical]/carousel:flex-col`,
        e,
      ),
      ...t,
    }),
  });
});
Ce.displayName = `CarouselContent`;
var we = o.forwardRef(({ className: e, ...t }, n) =>
  (0, W.jsx)(`div`, {
    ref: n,
    role: `group`,
    "aria-roledescription": `slide`,
    "data-slot": `carousel-item`,
    className: r(`min-w-0 shrink-0 grow-0 basis-full`, e),
    ...t,
  }),
);
we.displayName = `CarouselItem`;
var q = `inline-flex size-8 shrink-0 items-center justify-center rounded-full border border-border bg-background touch-manipulation transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50`,
  J = o.forwardRef(({ className: e, ...t }, n) => {
    let { scrollPrev: a, canScrollPrev: o } = G();
    return (0, W.jsxs)(`button`, {
      ref: n,
      type: `button`,
      "data-slot": `carousel-previous`,
      className: r(q, e),
      disabled: !o,
      onClick: a,
      ...t,
      children: [
        (0, W.jsx)(i, { className: `size-4` }),
        (0, W.jsx)(`span`, {
          className: `sr-only`,
          children: `Previous slide`,
        }),
      ],
    });
  });
J.displayName = `CarouselPrevious`;
var Y = o.forwardRef(({ className: e, ...t }, n) => {
  let { scrollNext: i, canScrollNext: o } = G();
  return (0, W.jsxs)(`button`, {
    ref: n,
    type: `button`,
    "data-slot": `carousel-next`,
    className: r(q, e),
    disabled: !o,
    onClick: i,
    ...t,
    children: [
      (0, W.jsx)(a, { className: `size-4` }),
      (0, W.jsx)(`span`, { className: `sr-only`, children: `Next slide` }),
    ],
  });
});
Y.displayName = `CarouselNext`;
var Te = o.forwardRef(({ className: e, ...t }, n) => {
  let { api: i } = G(),
    [a, s] = o.useState(0),
    [c, l] = o.useState([]),
    u = o.useCallback((e) => {
      e && l(e.scrollSnapList());
    }, []),
    d = o.useCallback((e) => {
      e && s(e.selectedScrollSnap());
    }, []);
  return (
    o.useEffect(() => {
      if (i)
        return (
          u(i),
          d(i),
          i.on(`reInit`, u),
          i.on(`reInit`, d),
          i.on(`select`, d),
          () => {
            (i.off(`reInit`, u), i.off(`reInit`, d), i.off(`select`, d));
          }
        );
    }, [i, u, d]),
    c.length === 0
      ? null
      : (0, W.jsx)(`div`, {
          ref: n,
          "data-slot": `carousel-dots`,
          className: r(`flex items-center justify-center gap-2`, e),
          ...t,
          children: c.map((e, t) =>
            (0, W.jsx)(
              `button`,
              {
                type: `button`,
                "data-slot": `carousel-dot`,
                "data-active": t === a ? `` : void 0,
                "aria-label": `Go to slide ${t + 1}`,
                "aria-current": t === a ? `true` : `false`,
                className: r(
                  `size-2 rounded-full transition-all duration-300`,
                  `bg-primary/20 hover:bg-primary/40 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-background`,
                  `data-active:w-6 data-active:bg-primary`,
                ),
                onClick: () => i?.scrollTo(t),
              },
              t,
            ),
          ),
        })
  );
});
Te.displayName = `CarouselDots`;
export { Y as a, we as i, Ce as n, J as o, Te as r, K as t };
