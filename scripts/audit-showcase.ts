...
for (const uiFile of uiSourceFiles) {
  ...
  const showcaseFile = showcaseSourceFiles.find(f => f.getBaseNameWithoutExtension() === baseName);
  ...
  for (const [name, declarations] of uiFile.getExportedDeclarations()) {
    if (/^[A-Z]/.test(name) && !name.includes("Context") && !name.includes("Provider") && !name.includes("Overlay") && !name.includes("Portal")) {
  ...
  const cvaCalls = uiFile.getDescendantsOfKind(SyntaxKind.CallExpression).filter(c => c.getExpression().getText() === "cva");
          ...
          if (variantsObj) {
            for (const variantProp of variantsObj.getProperties()) {
              if (variantProp.getKind() === SyntaxKind.PropertyAssignment) {
                ...
                const variantValuesObj = variantProp.getInitializerIfKind(SyntaxKind.ObjectLiteralExpression);
                if (variantValuesObj) {
                  variants[variantName] = variantValuesObj.getProperties().map(p => p.getName().replace(/['"]/g, ''));
                }
              }
            }
          }
  ...
  const monolithicFile = monolithicSourceFiles.find(f => f.getBaseNameWithoutExtension() === baseName);
  if (monolithicFile) {
    for (const [name, declarations] of monolithicFile.getExportedDeclarations()) {
      if (/^[A-Z]/.test(name) && !name.includes("Context") && !name.includes("Provider") && !exportedComponents.includes(name)) {
          const decl = declarations[0];
          if (decl && decl.getKind() !== SyntaxKind.TypeAliasDeclaration && decl.getKind() !== SyntaxKind.InterfaceDeclaration) {
            exportedComponents.push(name);
          }
  ...
  if (showcaseFile) {
    ...
    const usedTags = new Set(jsxElements.map(el => el.getTagNameNode().getText()));
    
    // Check components
    for (const comp of exportedComponents) {
      if (!usedTags.has(comp)) {
        const isNamespacedUsed = Array.from(usedTags).some(tag => tag.includes(`.${comp}`) || tag.endsWith(comp));
        if (!isNamespacedUsed) {
    ...
    for (const [vName, vValues] of Object.entries(variants)) {
      ...
      for (const val of vValues) {
        ...
        if (!usedValsStr.includes(val) && !usedValsStr.includes(`"${val}"`) && !usedValsStr.includes(`'${val}'`)) {
...
fs.writeFileSync("/Users/y/.gemini/antigravity-ide/brain/b74152fa-bc73-4d65-9843-c65a49e0b5c5/showcase_coverage_report.md", md);
console.log("Report generated at /Users/y/.gemini/antigravity-ide/brain/b74152fa-bc73-4d65-9843-c65a49e0b5c5/showcase_coverage_report.md");
