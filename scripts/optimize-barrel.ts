...
for (const file of filesToProcess) {
  ...
  for (const imp of imports) {
    ...
    if (specifier === "@/index" || specifier === "@/preset" || specifier === "@/preset;") {
      const namedImports = imp.getNamedImports();
      const importsByPath = new Map<string, {name: string, alias?: string}[]>();
      ...
      for (const named of namedImports) {
        ...
        if (sourcePath) {
          ...
          console.warn(`Export not found for ${name} in index.ts/preset.ts for file ${file.getFilePath()}`);
      ...
      for (const [sourcePath, namedList] of importsByPath) {
        if (sourcePath === "@/index" || sourcePath === "@/preset" || sourcePath === "@/preset;") continue; // skip unmapped to avoid infinite loop or just add them back if we want to preserve. Wait, if I skip them, they are lost! 
        // Let's add them back correctly if they weren't found.
        file.addImportDeclaration({
          moduleSpecifier: sourcePath,
          namedImports: namedList.map(n => n.alias ? { name: n.name, alias: n.alias } : { name: n.name }),
        });
      }
      ...
      if (importsByPath.has(specifier)) {
         file.addImportDeclaration({
            ...
            namedImports: importsByPath.get(specifier)!.map(n => n.alias ? { name: n.name, alias: n.alias } : { name: n.name }),
         });
      }
...
console.log(`Processed ${filesToProcess.length} files. Changed imports in ${totalChanged} files.`);
