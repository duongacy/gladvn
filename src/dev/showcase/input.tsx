  ...
  return (
    <div className="space-y-10 mt-6">
      <ShowcaseDocs>
        ...
        <DocsP>
          Bản Macro (<DocsCode>InputPreset</DocsCode>) đóng gói sẵn toàn bộ các
          thành phần thường thấy của một Form Control: Nhãn (Label), Mô tả
          (Description), Lỗi (Error Message) và phần trang trí (Adornments).
          Việc này giúp giảm thiểu lượng code lặp đi lặp lại khi xây dựng form.
        </DocsP>

        <DocsH3>Tự động hoá Layout</DocsH3>
        <DocsUl>
          <DocsLi>
            Tự động bọc Input vào trong <DocsCode>InputGroup</DocsCode> nếu bạn
            truyền <DocsCode>startAdornment</DocsCode> hoặc{" "}
    ...
    <div className="space-y-10 mt-6">
      <ShowcaseDocs>
        ...
        <DocsP>
          ...
          <DocsCode>input</DocsCode> duy nhất. Để có được một Form Control đúng
          chuẩn WCAG, bạn cần phải tự bọc nó bằng <DocsCode>Field</DocsCode>,
          kết nối <DocsCode>FieldLabel</DocsCode> và{" "}
          <DocsCode>FieldDescription</DocsCode>.
        </DocsP>
        <DocsP>
          Dùng Micro khi bạn cần layout form dị biệt (ví dụ: label nằm ngang bên
          trái input) hoặc khi cần nhúng Input vào một component phức tạp (như
          dropdown tìm kiếm).
        </DocsP>
      </ShowcaseDocs>
      ...
      >
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="rounded-xl border border-border bg-card p-5 flex flex-col gap-4">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Bạn cần một ô nhập <strong>"Tiêu đề"</strong> với label, gợi ý và
              có thể hiện lỗi. Bạn muốn viết càng ít code càng tốt.
            </p>
            <div className="rounded-lg bg-muted/50 p-3">
              <InputPreset
                size="sm"
            ...
            <div className="rounded-md border border-green-500/20 bg-green-500/5 px-3 py-2">
              <p className="text-xs font-medium text-green-700 dark:text-green-400">
                ✅ Dùng <code className="font-mono">InputPreset</code> — Label +
                description + error tích hợp sẵn, chỉ 1 dòng code thay vì bọc
                Field loằng ngoằng.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5 flex flex-col gap-4">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Bạn cần ô nhập tiền tệ và chèn 1 button "Tối đa" vào góc phải của
              InputGroup. Macro chỉ hỗ trợ chuỗi chữ hoặc icon cơ bản.
            </p>
            <div className="rounded-lg bg-muted/50 p-3">
              <InputGroup size="sm" className="w-full pr-1">
                <InputGroupAddon>
                ...
                <InputGroupInput placeholder="0" type="number" />
                <button className="h-5 px-2 rounded-sm bg-primary/10 text-primary text-[10px] font-semibold hover:bg-primary/20 transition-colors ml-2">
                  MAX
                </button>
              </InputGroup>
            </div>
            <div className="rounded-md border border-blue-500/20 bg-blue-500/5 px-3 py-2">
              <p className="text-xs font-medium text-blue-700 dark:text-blue-400">
                ✅ Dùng <code className="font-mono">InputGroup</code> (Micro) —
                Toàn quyền kiểm soát slot addon, nhét bất cứ thứ gì vào.
              </p>
            </div>
          </div>
        </div>
