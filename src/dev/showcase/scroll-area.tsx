...
export default function ScrollAreaShowcase() {
  return (
    <div className="space-y-10">
      <SectionHeader title="Scroll Area" description="Augments native scroll functionality for custom, cross-browser styling." />

      <ExampleGrid columns={2}>
        <ExampleSection label="Vertical" description="Scrollable list of items.">
        ...
        <ExampleSection label="Horizontal" description="Horizontal scrolling gallery.">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="w-[150px] shrink-0">
                  <div className="overflow-hidden rounded-md bg-muted aspect-[3/4] mb-2 flex items-center justify-center">
                    <span className="text-xs text-muted-foreground">Artwork {i + 1}</span>
                  </div>
                  <p className="text-sm font-medium">Title {i + 1}</p>
                  <p className="text-xs text-muted-foreground">Artist {i + 1}</p>
                </div>
              ))}
            </div>
