...
function SwitchForm({ size }: { size: Size }) {
  ...
  return (
    <form onSubmit={form.handleSubmit((v) => alert(JSON.stringify(v)))} className="w-full space-y-6">
      ...
      />
      <Button type="submit" size={size}>Lưu thay đổi</Button>
    </form>
  );
}
  ...
  return (
    <div className="space-y-10 mt-6">
      <ShowcaseDocs>
        ...
        <DocsP>
          <DocsCode>SwitchPreset</DocsCode> đóng gói Switch + Label + Description + Error Message. Nó tự động căn chỉnh khoảng cách và liên kết ID. Dùng trong 95% trường hợp làm màn hình cài đặt (Settings) hoặc form yêu cầu bật/tắt tính năng.
        </DocsP>
      </ShowcaseDocs>

      <ExampleGrid columns={2}>
        <ExampleSection label="Tiêu chuẩn" description="Công tắc cơ bản có nhãn và mô tả."
...
`}>
        ...
        <ExampleSection label="Trạng thái Lỗi (Error)" description="Báo lỗi khi bật/tắt thất bại."
...
`}>
      ...
      <ExampleGrid columns={2}>
        <ExampleSection label="Khóa / Bất hoạt (Disabled)" description="Người dùng không thể tương tác với công tắc."
          codeString={`<div className="w-full flex flex-col gap-6">
  <SwitchPreset
    size={globalSize}
...
`}>
        ...
        <ExampleSection label="Nhãn phức tạp (Custom Content)" description="Truyền React Node vào Label để tạo giao diện phong phú."
          codeString={`<div className="w-full">
  <SwitchPreset
    size={globalSize}
    label={
        ...
        >
          <div className="w-full flex flex-col gap-4">
            ...
            <p className="text-sm text-muted-foreground">
              Trạng thái: <span className="font-mono font-bold text-foreground">{checked ? "BẬT" : "TẮT"}</span>
            </p>
          </div>
        </ExampleSection>
  ...
  return (
    <div className="space-y-10 mt-6">
      <ShowcaseDocs>
        ...
        <DocsP>
          Dùng <DocsCode>Switch</DocsCode> và <DocsCode>SwitchThumb</DocsCode> nguyên thuỷ khi bạn cần một layout không theo chuẩn <DocsCode>SwitchPreset</DocsCode> (ví dụ: Switch nằm bên phải Label thay vì nằm bên trái).
        </DocsP>
      </ShowcaseDocs>

      <ExampleGrid columns={2}>
        <ExampleSection label="Cơ bản (Primitive)" description="Kết nối thủ công Switch và SwitchThumb."
...
`}>
        ...
        <ExampleSection label="Layout Đảo ngược (Reverse)" description="Dùng Field với orientation='horizontal' để đẩy Switch sang bên phải."
  ...
  <Field orientation="horizontal" size={globalSize} className="justify-between">
    ...
    <Switch id="switch-notif" size={globalSize} defaultChecked><SwitchThumb /></Switch>
  </Field>
  <Field orientation="horizontal" size={globalSize} className="justify-between">
    <FieldLabel htmlFor="switch-dark" className="font-normal cursor-pointer">Chế độ tối (Dark Mode)</FieldLabel>
    <Switch id="switch-dark" size={globalSize}><SwitchThumb /></Switch>
  </Field>
</div>
`}>
      ...
      <ExampleGrid columns={2}>
        <ExampleSection label="Khóa (Disabled)" description="Các công tắc đảo ngược nhưng bị vô hiệu hoá."
          codeString={`<div className="w-full space-y-4">
  <Field orientation="horizontal" size={globalSize} className="justify-between">
    <FieldLabel htmlFor="switch-d1" className="font-normal text-muted-foreground">Bluetooth</FieldLabel>
    <Switch id="switch-d1" disabled size={globalSize} defaultChecked><SwitchThumb /></Switch>
  </Field>
  <Field orientation="horizontal" size={globalSize} className="justify-between">
    <FieldLabel htmlFor="switch-d2" className="font-normal text-muted-foreground">Vị trí (GPS)</FieldLabel>
    <Switch id="switch-d2" disabled size={globalSize}><SwitchThumb /></Switch>
  </Field>
</div>
`}>
          <div className="w-full space-y-4">
        ...
        <ExampleSection label="Trạng thái Lỗi thủ công" description="Tự gắn aria-invalid vào Switch."
          codeString={`<Field orientation="horizontal" data-invalid={true} size={globalSize} className="justify-between items-start">
  ...
  <Switch id="switch-err" size={globalSize} aria-invalid={true} defaultChecked><SwitchThumb /></Switch>
</Field>
`}>
          <Field orientation="horizontal" data-invalid={true} size={globalSize} className="justify-between items-start">
            <div className="grid gap-1.5">
              <FieldLabel htmlFor="switch-err" className="font-normal cursor-pointer">Tự động sao lưu</FieldLabel>
              <FieldError>Máy chủ sao lưu đang offline.</FieldError>
            </div>
...
export default function SwitchShowcase() {
  ...
  return (
    <Showcase
      ...
      tabs={[
        { label: "Micro (Primitive)", content: <SwitchMicroShowcase globalSize={globalSize} /> },
        { label: "Macro (Preset)", content: <SwitchMacroShowcase globalSize={globalSize} /> },
      ]}
    />
  );
}
