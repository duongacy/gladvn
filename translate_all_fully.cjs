...
const files = fs.readdirSync(dir).filter(f => f.endsWith(".tsx") && f !== "accordion.tsx");
  ...
  if (replacements.length > 0) {
    ...
    function visitFuncs(node) {
      if (ts.isImportDeclaration(node)) {
        if (node.moduleSpecifier.text.includes("dev-context") && node.importClause && node.importClause.getText(sf2).includes("useI18n")) {
          hasImport = true;
        }
      }
